...
function combustionBridgeProxy() {
  ...
  return {
    ...
    configureServer(server) {
      // Single mount point — Connect strips the prefix from req.url,
      // so we route internally based on the remaining pathname.
      server.middlewares.use('/api/bridge', async (req, res, next) => {
        const url = new URL(req.url, 'http://localhost');
        // After prefix stripping: /probe?host=... or /get?host=...&path=...
